...
public class RoundedDrawable extends Drawable {
  ...
  private ScaleType mScaleType = ScaleType.FIT_CENTER;
    ...
    /** edit  by liht on 2016/8/15 9:18 对每个角单独处理 */
    private boolean isCornerLT = true;
    private boolean isCornerRT = true;
    private boolean isCornerLB = true;
    private boolean isCornerRB = true;
  ...
  protected boolean onStateChange(int[] state) {
    ...
    /** edit  by liht on 2016/8/15 9:20 */
    public RoundedDrawable setCorners(boolean isCornerLT, boolean isCornerRT, boolean isCornerLB, boolean isCornerRB){
        this.isCornerLT = isCornerLT;
        this.isCornerRT = isCornerRT;
        this.isCornerLB = isCornerLB;
        this.isCornerRB = isCornerRB;
        return this;
    }
  ...
  public void draw(Canvas canvas) {
    ...
    if (mOval) {
      ...
      if (mBorderWidth > 0) {
        ...
        canvas.drawRoundRect(mBorderRect, mCornerRadius, mCornerRadius, mBorderPaint);
          ...
          drawRoundRect(canvas, mDrawableRect, mCornerRadius, mBitmapPaint);
      }
    }
  }
    ...
    private void drawRoundRect(Canvas canvas, RectF rect, float r, Paint paint){
        ...
        if (r > 0){
            ...
            if (!isCornerLT){
                canvas.drawRect(mDrawableRect.left, mDrawableRect.top, cx, cy, paint);
            }
            if (!isCornerRT){
                canvas.drawRect(cx, mDrawableRect.top, mDrawableRect.right, cy, paint);
            }
            if (!isCornerLB){
                canvas.drawRect(mDrawableRect.left, cy, cx, mDrawableRect.bottom, paint);
            }
            if (!isCornerRB){
                canvas.drawRect(cx, cy, mDrawableRect.right, mDrawableRect.bottom, paint);
            }
        }
    }
